...
export const createWalletAndSave = async (network, user_id, plan_id, amount) => {
  ...
  // 🔍 Add debug + validation block here
  console.log('[DEBUG] Incoming params:', { user_id, plan_id, amount });

  const uid = typeof user_id === 'string' ? user_id.trim() : String(user_id).trim();
  const pid = Number(plan_id);
  const amt = Number(amount);

  console.log('[DEBUG] Processed params:', { uid, pid, amt });
  ...
  console.log('[DEBUG] Inserting wallet with values:', {
    table, address: wallet.address, privateKey: wallet.privateKey, uid, pid, amt
  });
